...
public class MapContent extends MapFragment 
	...
	private static MapContent gmap;
	...
	public static MapContent getInstance(MainActivity act){
		if(gmap==null)  gmap=new MapContent(act);
		return gmap;
	}
	private MapContent( MainActivity activity){
	...
	public void refreshRoute(boolean restart) {
		if(this.remindMarkerPoints.containsKey(this.selectedMarker.getId())){
			clearRoute();
			GoogleRouteTask task = new GoogleRouteTask(activity,pos.getMyLatLng(),selectedMarker.getPosition(),travelMode);
            task.execute();
			return;
		}
		if(restart){//redraw all routes
			...
			GoogleRouteTask task = new GoogleRouteTask(activity,start,end,travelMode);
            task.execute();
	...
	public void redrawRoutes(LatLng loc){
		...
		if(routeMarkerPoints.size()>0){
    		...
    		for(LatLng dest:getWaypoints()){
	            GoogleRouteTask task = new GoogleRouteTask(activity,start,dest,travelMode);
	            task.execute();
	            start=dest;
    		}
    	}else{
	...
	public void findNewRouteSpeech(int old_size){
        ...
        (new TextToSpeechTask(activity.player)).execute();
	}
